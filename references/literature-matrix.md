...
| Scalable Bloom Filters | 2007 | 可增长 Bloom filter 组件序列 | 最终容量不预知；单组件有容量 | 支持 | 需查组件数 | 待核验 | 待核验 | 摘要称保证最大 FP | 待核验 | 工程/算法对照，[1] | 仅看摘要 | Elsevier 摘要 |
| Dynamic Bloom Filters | 2006 | 多 Bloom filter 的动态集合 | 使用预定义阈值 | 支持 | 待核验 | 待核验 | 摘要称实际规模不偏离阈值时可接受 | 一侧错误 | 待核验 | 动态工程对照，[16] | 仅看摘要 | 论文摘要 |
...
| Quotient filter | 2012 | fingerprint AMQ / 外存友好 | 基础表容量预设，可 resize | 支持 | 实验/理论待核 | 支持 | 待核验 | 一侧错误 | 待核验 | 原文未引用的工程对照 | 仅看摘要 | PVLDB 摘要 |
| Cuckoo filter | 2014 | fingerprint AMQ | 通常按容量/负载设表 | 支持 | 实践快；理论待核 | 支持 | 论文称在部分 FPR 范围实践更省 | 一侧错误 | 插入失败/最坏界待核 | 原文未引用的工程对照 | 仅看摘要 | CoNEXT 论文首页/摘要 |
