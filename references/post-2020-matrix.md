...
> **当前状态（2026-07-30）**：E1/E2 项是明确的证据等级，不是遗漏的成员任务；正文仅按各行已核验范围使用，2026 预印本保留提交前版本复核要求。
...
- [x] Aleph PDF p.12 的 LYY 比较已按 E3 范围进入引用审计和 C 章，未写成全面取代。
- 最终状态：STOC 2024 的模型区别已按 E3 证据写入矩阵和正文；小组不再为口头说明另建记录文件。
- [x] Resizable Retrieval v1 pp.2、20--21 与 Corollary 3.14 已按引用审计中的 E3 限定写入正文。
- 最终未采用 InfiniFilter 承担确定性比较，因此无需补充该项全文论证。
- Li 2023/2024 未进入 Review 的确定性比较句，E1 状态仅保留在研究矩阵。
