...
> **历史文件说明（2026-07-30 回填）**：本提纲保留 Day 1–2 的待核查与待补项作为写作轨迹。其中 A/B/C 三章、PSW 下界、概率/空间证明、相关研究与引用审计已由 Day 5/6 产物替代；当前成稿以 `drafts/review-integrated.md` v1.0 为准。

初建日期：2026-07-17
最近更新：2026-07-28
状态：Day 4 三人材料整合版。A 已补齐概率、failure、空间和渐近条件核查；B 已完成抽象伪代码、迁移与时间证明框架；C 已完成后续研究矩阵及可追溯性审阅。阶段下标、字面常数 10、位级空间峰值仍保留为 Q1/Q3，提纲不是终稿。
...
- Aleph Filter（2024）：与未知增长直接相关，支持删除和工程化无限扩展；部分清理为 amortized，不能等同本文 whp worst-case 保证。
- Kuszmaul--Walzer（STOC 2024）：删除型 fully dynamic filter 的线性额外空间下界，与本文 insertion-only unknown-size 模型不同。
- Resizable Retrieval（2026 预印本）：支持删除且空间按当前 `n`，但含不同的 `U/n`、`U^δ` 和 whp 参数，作为直接新进展而非全面替代。
- InfiniFilter 与 Li 等工作在全文核验不足处继续标候选/理论旁支。
- 比较表至少包含：
  - 问题模型；
  - 是否需要容量上界；
  - 是否支持删除；
